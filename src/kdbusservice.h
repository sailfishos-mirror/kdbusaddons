 ...
 * \brief KDBusService takes care of registering the current process with D-Bus.
 *
 ...
 * the \l {http://standards.freedesktop.org/desktop-entry-spec/desktop-entry-spec-latest.html}
 ...
 * various instances; for example: \e org.kde.konqueror-12345.
 *
 ...
 *
 * Applications that set the D-Bus activation entry (\c DBusActivatable=true) in
...
public:
     ...
     * \value [since 5.65] Replace
     * Indicates that if there's already a unique service running,
     * to be quit and replaced with our own.
     * If exported, it will try first quitting the service calling
     * \c org.qtproject.Qt.QCoreApplication.quit,
     ...
     * application name and organization domain under the given \a parent.
     *
     * The D-Bus service name is the reversed organization domain, followed by
     * the application name.  If \a options includes the \c Multiple flag, the
     * application PID will be appended.  For example:
     * \code
     * app.setApplicationName("kuiserver");
     * app.setOrganizationDomain("kde.org");
     * \endcode
     * The above will make KDBusService register as \c org.kde.kuiserver in \c Unique
     ...
     * Qt::DirectConnection.
     *
     * \a value  The exit value for the duplicate instance.
     */
...
Q_SIGNALS:
     ...
     * D-Bus method to trigger this signal. In this case, \a arguments will be empty.
     *
     * In single-window applications, the connected signal should typically
     * raise the window.
     *
     * \a arguments  The arguments the executable was called with,
     * starting with the executable file name.
     * See QCoreApplication::arguments(). This can be empty.
     *
     * \a workingDirectory The directory from which the executable is called.
     * This can be empty.
     *
     ...
     * for details.
     *
     * \a uris  The URLs of the files to open.
     */
    void openRequested(const QList<QUrl> &uris);

    /*!
     * Signals that an application action \a actionName should be triggered
     * with the given \a parameter.
     *
     * This signal is emitted to request handling of the respective method of the D-Bus activation.
     * For GUI applications, the signal handler also needs to deal with any platform-specific startup ids
     * and make sure the mainwindow is shown as well as request its activation from the window manager.
...
||||||| parent of c045a2f (QDoc KDBusService)
    /**
     * Manually unregister the given serviceName from D-Bus.
=======
    /*!
     * Manually unregister the given serviceName from D-Bus.
>>>>>>> c045a2f (QDoc KDBusService)
     */
    void unregister();
